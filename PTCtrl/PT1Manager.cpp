...
BOOL CPT1Manager::CloseTuner(int iID)
{
	...
	static DWORD __inline dur (DWORD s=0, DWORD e=GetTickCount())
	{
		// duration ( s -> e )
...
{
	_OutputDebugString(L"CPT1Manager::SetCh: iId=%d ulCh=%d dwTSID=%d\n",iID,ulCh,dwTSID) ;
	const DWORD MAXDUR_FREQ = 1000; //周波数調整に費やす最大時間
	const DWORD MAXDUR_TMCC = 1500; //TMCC取得に費やす最大時間
	const DWORD MAXDUR_TSID = 3000; //TSID設定に費やす最大時間
